  ...
  setTimeout(() => {
    try {
      switch (layout) {
        ...
        case 'faculty':        renderFaculty(area, sheets, sheetName, sectionKey); break;
...
function renderFaculty(area, sheets, sheetName, sectionKey) {
  const rows = sheets[sheetName] || [];
  let html = `<h2 class="section-heading">${esc(sheetName)} <span class="section-count">${rows.length} 位</span></h2><div class="faculty-grid">`;
  rows.forEach((r, idx) => {
    const photo = r['照片URL'] || '';
    html += `<div class="faculty-card" data-act="openFaculty" data-sec="${esc(sectionKey)}" data-idx="${idx}" data-sheet="${esc(sheetName)}">`;
    if (photo) {
    ...
    if (r['Email']) html += `<div class="fc-email">✉ ${esc(r['Email'])}</div>`;
    html += '</div></div>';
  });
  area.innerHTML = html + '</div>';
}
...
function renderAssistant(area, sheets, sheetName) {
  ...
  rows.forEach(r => { html += '<tr>' + cols.map((c,i)=>`<td${i===last?' class="wrap-col"':''}>${esc(String(r[c]||''))}</td>`).join('') + '</tr>'; });
  area.innerHTML = html + '</tbody></table></div>';
}
...
function renderResearch(area, sheets, sheetName) {
  ...
  researchRows.forEach((r, idx) => {
    const photo = r['照片URL']||'';
    html += `<div class="research-card" data-act="openResearch" data-idx="${idx}">`;
    if (photo) {
...
function renderCourseUG(area, sheets, sheetName) {
  ...
  grades.forEach((g,i) => {
    ...
    if (i === 0) html += buildEmiSection(sheets['大學部EMI課程地圖'], '大學部 EMI 課程地圖');
    html += '</div>';
  });
  area.innerHTML = html;
  initTabs(area);
...
function newsItemHTML(r, showBadge) {
  ...
  <div class="news-item-row" data-act="newsToggle">
...
<div class="news-toolbar">
  <select class="news-year-sel" id="news-year"><option value="">全部年份</option>${years.map(y=>`<option value="${y}">${y}</option>`).join('')}</select>
</div>
...
function newsToggle(rowEl) {
  rowEl.closest('.news-item').classList.toggle('open');
}
...
async function init() {
  ...
  SECTIONS.filter(s => s.show).forEach(sec => {
    LOADP[sec.key] = loadXlsx(sec.xlsxFile)
      .then(d => {
        DATA[sec.key] = d;
        if (document.getElementById('vb-text').textContent === '載入中…')
