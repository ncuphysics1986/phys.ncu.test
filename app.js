...
const SECTIONS = [
  ...
  { key:'alumni',   label:'系友專區', xlsxFile:'alumni.xlsx',      show:true },   /* B方案 2026-07-03 */
...
async function applyWebsiteConfig() {
  try {
    ...
    (wb['網頁架構'] || []).forEach(row => {
      ...
      if (!sec) return;
      if (row['選單分類_en']) sec.labelEn = String(row['選單分類_en']).trim();
...
function setVersionBadge(lastMod) {
  ...
  if (!lastMod) { el.textContent = t('已載入'); return; }
  try {
    const d = new Date(lastMod);
    if (isNaN(d)) { el.textContent = t('已載入'); return; }
    const p = n => String(n).padStart(2,'0');
    el.textContent = `${t('已更新')}　${d.getFullYear()}-${p(d.getMonth()+1)}-${p(d.getDate())} ${p(d.getHours())}:${p(d.getMinutes())}`;
  } catch(_) { el.textContent = t('已載入'); }
}
...
/* ════════════════════════════════════════════════════
   雙語引擎（Phase 2, 2026-07-03）
   ─ 路由：#en/分區/分頁（中文版無前綴，既有連結不變）
   ─ 資料：載入後以「欄名_en」覆蓋「欄名」，可逆（切回中文還原）
   ─ 政策：英文版隱藏 news / highschool 分區（EN_HIDDEN_SECTIONS）
════════════════════════════════════════════════════ */
let LANG = 'zh';
const EN_HIDDEN_SECTIONS = new Set(['news', 'highschool']);
const TITLE_SUFFIX = { zh: '｜國立中央大學物理學系', en: ' | Department of Physics, NCU' };
const T_EN = {
  '讀取設定檔與資料中':'Loading site data','載入資料中':'Loading',
  ...
  '國外學者來訪':'International Visitors','來訪者':'Visitor','來訪國家':'Country','職稱':'Title','來訪單位':'Affiliation','邀請人':'Host','日期':'Date',
  ' 位':'',' 筆':'',' 間':'',
  '歷史沿革':'History','教育目標與核心能力':'Educational Goals & Core Competencies','系所活動':'Events & Gallery',
  '研究成果 ':'Research Results ',' 項':'','大學部課程':'Undergraduate Courses','學生活動空間':'Student Space',
  '學生獲獎紀錄':'Student Awards','姓名':'Name','身份':'Role','指導教授':'Advisor','得獎日期':'Date','得獎名稱':'Award',
  '研究生論文':'Theses','學生':'Student','指導老師':'Advisor','學位':'Degree','學年度':'Year','論文題目':'Thesis Title',
  '學生國外差旅':'Student Travel','教師':'Advisor','出國期間':'Period','國家':'Country','經費來源':'Funding','出國目的':'Purpose',
  '相關學程':'Related Programs','物理系刊':'Physics Newsletter',
};
const t = s => (LANG === 'en' && (s in T_EN)) ? T_EN[s] : s;
function langHash(sec, pg) { return (LANG === 'en' ? '#en/' : '#') + sec + '/' + pg; }
function parseLangHash(h) { const seg = h.split('/'); let en = false;
  if (seg[0] === 'en') { en = true; seg.shift(); } return { en, seg }; }
/* 可逆覆蓋：欄名_en 非空 → 蓋到 欄名；原值暫存於 row['\u0000zh']，切回中文即還原 */
function overlayRows(sheets, en) {
  for (const n in sheets) { const rows = sheets[n]; if (!Array.isArray(rows)) continue;
    for (const r of rows) {
      if (en) { let st = r['\u0000zh'];
        for (const k in r) { if (k.length > 3 && k.slice(-3) === '_en' && r[k] !== '' && r[k] != null) {
          const b = k.slice(0, -3); if (!(b in r)) continue;
          if (!st) st = r['\u0000zh'] = {}; if (!(b in st)) st[b] = r[b]; r[b] = r[k]; } } }
      else { const st = r['\u0000zh']; if (st) { for (const b in st) r[b] = st[b]; delete r['\u0000zh']; } }
    } }
}
function applyLangToSections() { const en = LANG === 'en';
  SECTIONS.forEach(s => { if (s._zh === undefined) s._zh = { label: s.label, show: s.show };
    s.label = (en && s.labelEn) ? s.labelEn : s._zh.label;
    s.show  = en ? (s._zh.show && !EN_HIDDEN_SECTIONS.has(s.key)) : s._zh.show; });
}
function setLang(en) { if ((LANG === 'en') === !!en) return;
  LANG = en ? 'en' : 'zh';
  document.documentElement.lang = en ? 'en' : 'zh-TW';
  applyLangToSections();
  for (const k in DATA) overlayRows(DATA[k], en);
  const tg = document.getElementById('lang-toggle'); if (tg) tg.textContent = en ? '中' : 'EN';
  buildNav(); buildMobileNav();
}
  ...
  SECTIONS.filter(s => s.show).forEach(sec => {
    ...
    a.onclick = async e => {
      ...
      if (!DATA[sec.key] && LOADP[sec.key]) {
        document.getElementById('list-view').innerHTML = `<div class="loading">${t('載入資料中')}</div>`;
...
function buildMobileNav() {
  ...
  if (!body) return;
  body.innerHTML = '';
  { const lb = document.createElement('button');            /* 雙語：抽屜內切換鈕 */
    lb.type = 'button'; lb.className = 'mnav-lang';
    lb.textContent = LANG === 'en' ? '中文版' : 'English';
    lb.onclick = () => document.getElementById('lang-toggle')?.click();
    body.appendChild(lb); }
  SECTIONS.filter(s => s.show).forEach(sec => {
...
async function navigateTo(sectionKey, pageId) {
  ...
  document.title = page.label + (LANG === 'en' ? TITLE_SUFFIX.en : TITLE_SUFFIX.zh);
  ...
  /* Update URL hash */
  const newHash = langHash(sectionKey, pageId);
...
function renderTimeline(area, sheets, sheetName, desc) {
  ...
  const eras = [], events = [];
  /* 雙語：本表表頭不在第 1 列（橫幅型），列鍵為 __EMPTY_x，通用覆蓋搆不到；
     改經 sheetDataRows 的欄名映射讀「欄名_en」，空值回退中文 */
  const G = (r, c) => { if (LANG === 'en') { const v = String(data.at(r, c + '_en') ?? '').trim(); if (v) return v; }
    return String(data.at(r, c)).trim(); };
  data.rows.forEach(r => {
  ...
  let html = `<h2 class="section-heading">${t('歷史沿革')}</h2>`;
...
function renderText(area, desc) {
  if (!desc) { area.innerHTML = `<p style="color:var(--muted)">${t('（此頁面暫無內容）')}</p>`; return; }
...
function renderChairCards(area, sheets, sheetName, page) {
  ...
  let html = `<h2 class="section-heading">${esc(page.cfg['頁首大標題']||'')} <span class="section-count">${rows.length}${t(' 位')}</span></h2><div class="chair-grid">`;
  rows.forEach(r => {
    const photo = r['照片URL'] || '';
...
function renderFaculty(area, sheets, sheetName, sectionKey, page) {
  ...
  visible.forEach(({ r, idx }) => {
    ...
    const detailAttrs = detailable
      ? ` role="button" tabindex="0" aria-label="${LANG==='en'?`Details of ${esc(r['姓名']||'')}`:`${esc(r['姓名']||'')} 詳細資料`}" data-act="openFaculty" data-sec="${esc(sectionKey)}" data-idx="${idx}" data-sheet="${esc(sheetName)}"`
      : '';
...
<button class="fd-back" data-act="closeFaculty">${t('← 返回列表')}</button>
<div class="fd-layout">
  <div class="fd-sidebar">
    ...
    <div class="fd-contact">
      ${r['辦公室']     ? `<div>🏢 ${t('辦公室')}：${esc(r['辦公室'])}</div>`       : ''}
      ${r['研究室電話'] ? `<div>📞 ${t('研究室')}：${esc(r['研究室電話'])}</div>`   : ''}
      ${r['實驗室電話'] ? `<div>🔬 ${t('實驗室')}：${esc(r['實驗室電話'])}</div>`   : ''}
  ...
  <div class="fd-main">
    ...
    <div class="fd-tabs">
      <button class="fd-tab-btn active" data-act="switchFdTab" data-tab="fd-projs">${t('研究計畫')} (${projs.length})</button>
      <button class="fd-tab-btn"        data-act="switchFdTab" data-tab="fd-pubs">${t('研究著作')} (${pubs.length})</button>
    </div>
    <div id="fd-projs">
      ${projs.length
        ? `<table class="proj-table"><thead><tr><th>${t('計畫年度')}</th><th>${t('計畫名稱')}</th><th>${t('執行起迄')}</th></tr></thead><tbody>`
          + projs.map(p=>`<tr><td>${esc(String(p['計畫年度']||''))}</td><td>${esc(p['計畫名稱']||'')}</td><td>${esc(p['執行起迄']||'')}</td></tr>`).join('')
          + '</tbody></table>'
        : `<p style="color:var(--muted);font-size:.9rem">${t('暫無研究計畫資料')}</p>`}
    </div>
    <div id="fd-pubs" style="display:none">
      ${pubs.length
        ...
        : `<p style="color:var(--muted);font-size:.9rem">${t('暫無著作資料')}</p>`}
    </div>
  </div>
</div>`;
...
function renderStaff(area, sheets, sheetName) {
  ...
  let html = `<h2 class="section-heading">${esc(sheetName)} <span class="section-count">${rows.length}${t(' 位')}</span></h2><div class="table-wrap"><table class="data-table"><thead><tr>${cols.map(c=>`<th>${esc(c)}</th>`).join('')}</tr></thead><tbody>`;
  rows.forEach(r => {
...
function renderAssistant(area, sheets, sheetName) {
  ...
  const rows = (sheets[sheetName] || []).filter(rowEnabled);
  let html = `<h2 class="section-heading">${esc(sheetName)} <span class="section-count">${rows.length}${t(' 位')}</span></h2><div class="table-wrap"><table class="data-table nowrap-table"><thead><tr>${cols.map(c=>`<th>${esc(c)}</th>`).join('')}</tr></thead><tbody>`;
...
function renderCompetency(area, sheets, sheetName) {
  ...
  let html = `<h2 class="section-heading">${t('教育目標與核心能力')}</h2>`;
...
function renderActivities(area, sheets, sheetName) {
  ...
  let html = `<h2 class="section-heading">${t('系所活動')}</h2>`;
  catOrder.forEach(cat => {
...
function renderResearch(area, sheets, sheetName) {
  ...
  let html = `<h2 class="section-heading">${t('研究成果 ')}<span class="section-count">${researchRows.length}${t(' 項')}</span></h2><div class="research-grid">`;
  researchRows.forEach((r, idx) => {
    const photo = r['照片URL']||'';
    html += `<div class="research-card" role="button" tabindex="0" aria-label="${LANG==='en'?`Details of ${esc(r['標題']||'')}`:`${esc(r['標題']||'研究成果')} 詳細資料`}" data-act="openResearch" data-idx="${idx}">`;
    if (photo) {
...
function renderLab(area, sheets, sheetName) {
  const rows = sheets[sheetName]||[];
  let html = `<h2 class="section-heading">${t('實驗室 ')}<span class="section-count">${rows.length}${t(' 間')}</span></h2><div class="table-wrap"><table class="data-table"><thead><tr><th>${t('中文名稱')}</th><th>${t('英文名稱')}</th><th>${t('實驗室主持人')}</th><th>${t('實驗室網頁')}</th></tr></thead><tbody>`;
  rows.forEach(r => {
    const web = r['實驗室網頁']||'';
...
function renderAwards(area, sheets, sheetName) {
  ...
  let html = `<h2 class="section-heading">${t('教師獲獎紀錄')} <span class="section-count">${rows.length}${t(' 筆')}</span></h2><div class="table-wrap"><table class="data-table"><thead><tr><th>${t('獲獎年度')}</th><th>${t('主辦單位')}</th><th>${t('榮譽名稱')}</th><th>${t('獲獎人')}</th></tr></thead><tbody>`;
...
function renderVisitors(area, sheets, sheetName) {
  const rows = sheets[sheetName]||[];
  let html = `<h2 class="section-heading">${t('國外學者來訪')} <span class="section-count">${rows.length}${t(' 筆')}</span></h2><div class="table-wrap"><table class="data-table visitor-table"><colgroup><col style="width:20%"><col style="width:10%"><col style="width:10%"><col style="width:34%"><col style="width:11%"><col style="width:15%"></colgroup><thead><tr><th>${t('來訪者')}</th><th>${t('來訪國家')}</th><th>${t('職稱')}</th><th>${t('來訪單位')}</th><th>${t('邀請人')}</th><th>${t('日期')}</th></tr></thead><tbody>`;
...
function csImgFail(img) {
  ...
  if (!remaining.length) {
    carousel.outerHTML = `<p style="color:var(--muted);font-size:.88rem;padding:12px">${t('（此分類暫無照片）')}</p>`;
    return;
  }
...
function renderScholarship(area, sheets, sheetName) {
  ...
  html += '</div>';
  /* 獎學金公告：自公告總庫匯入（分類=獎學金）。政策：英文版不顯示公告，故略過本區塊 */
  if (LANG !== 'en') {
...
function renderSpaces(area, sheets, sheetName) {
  ...
  let html = `<h2 class="section-heading">${t('學生活動空間')}</h2><div class="spaces-grid">`;
  rows.forEach((r, idx) => {
...
function renderStudentAwards(area, sheets, sheetName) {
  ...
  let html = `<h2 class="section-heading">${t('學生獲獎紀錄')} <span class="section-count">${rows.length}${t(' 筆')}</span></h2><div class="table-wrap"><table class="data-table nowrap-table"><thead><tr><th>${t('姓名')}</th><th>${t('身份')}</th><th>${t('指導教授')}</th><th>${t('得獎日期')}</th><th>${t('得獎名稱')}</th></tr></thead><tbody>`;
...
function renderTheses(area, sheets, sheetName) {
  ...
  let html = `<h2 class="section-heading">${t('研究生論文')} <span class="section-count">${rows.length}${t(' 筆')}</span></h2><div class="table-wrap"><table class="data-table" style="table-layout:fixed;width:100%"><colgroup><col style="width:6em"><col style="width:7em"><col style="width:5.5em"><col style="width:5em"><col></colgroup><thead><tr><th>${t('學生')}</th><th>${t('指導老師')}</th><th style="white-space:nowrap">${t('學位')}</th><th>${t('學年度')}</th><th>${t('論文題目')}</th></tr></thead><tbody>`;
  rows.forEach(r => {
...
function renderStudentTravel(area, sheets, sheetName) {
  const rows = sheets[sheetName]||[];
  let html = `<h2 class="section-heading">${t('學生國外差旅')} <span class="section-count">${rows.length}${t(' 筆')}</span></h2><div class="table-wrap"><table class="data-table nowrap-table"><thead><tr><th>${t('學生')}</th><th>${t('教師')}</th><th>${t('身份')}</th><th>${t('出國期間')}</th><th>${t('國家')}</th><th>${t('經費來源')}</th><th>${t('出國目的')}</th></tr></thead><tbody>`;
...
function renderPrograms(area, sheets, sheetName) {
  const rows = sheets[sheetName]||[];
  let html = `<h2 class="section-heading">${t('相關學程')}</h2><div class="program-list">`;
  rows.forEach(r => {
...
function renderJournals(area, sheets, sheetName) {
  const rows = sheets[sheetName]||[];
  let html = `<h2 class="section-heading">${t('物理系刊')}</h2><div class="journal-grid">`;
  rows.forEach(r => {
...
async function renderPortal(area, sheets, sheetName) {
  ...
  area.innerHTML = html || `<p style="color:var(--muted)">${t('（此頁面暫無內容）')}</p>`;
...
async function init() {
  ...
  /* 雙語：自初始 hash 判定語言（在任何渲染之前），並綁定切換鈕 */
  const __init_lang = parseLangHash(location.hash.replace('#',''));
  if (__init_lang.en) { LANG = 'en'; document.documentElement.lang = 'en'; }
  const __lb = document.getElementById('lang-toggle');
  if (__lb) { __lb.textContent = LANG === 'en' ? '中' : 'EN';
    __lb.onclick = () => {
      setLang(LANG !== 'en');
      let sec = curSection;
      if (!SECTIONS.find(s => s.show && s.key === sec)) sec = SECTIONS.find(s => s.show)?.key;
      const pgs = getPages(sec) || [];
      const pg = (sec === curSection && curPageId && pgs.find(p => p.id === curPageId)) ? curPageId : pgs[0]?.id;
      if (pg) navigateTo(sec, pg);
    }; }

  document.getElementById('list-view').innerHTML = `<div class="loading">${t('讀取設定檔與資料中')}</div>`;

  await applyWebsiteConfig();
  if (LANG === 'en') applyLangToSections();

  /* 全部 xlsx 同時開始載入（不互相等待）；Promise 存入 LOADP */
  SECTIONS.filter(s => s.show).forEach(sec => {
    LOADP[sec.key] = loadXlsx(sec.xlsxFile)
      .then(d => {
        if (LANG === 'en') overlayRows(d, true);
        DATA[sec.key] = d;
  ...
  const [rawSec, rawPg, hashSub] = __init_lang.seg;
  ...
  window.addEventListener('popstate', () => {
    const { en, seg } = parseLangHash(location.hash.replace('#',''));
    if (en !== (LANG === 'en')) setLang(en);
    const [rawSec, rawPg, sub] = seg;
    ...
    else if (!s || !pg) {                       /* 語言切換或不完整 hash → 導向可見首頁 */
      const fk = SECTIONS.find(x => x.show)?.key;
      const fp = fk && getPages(fk)[0]?.id;
      if (fk && fp) navigateTo(fk, fp);
    }
  });
}
